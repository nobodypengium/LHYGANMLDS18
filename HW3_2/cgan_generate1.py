...
def save_imgs(generator,tags):
    import matplotlib.pyplot as plt

    r, c = 5, 5
    noise = np.random.normal(0, 1, (r * c, 100))
    label = np.random.randint(0, 119, r * c)
    ...
    for i in range(r):
        for j in range(c):
            ...
            axs[i, j].axis('off')
            axs[i, j].set_title(tags[label[cnt]].decode('utf8'))
            cnt += 1
...
if __name__ == '__main__':
    ...
    _, labels, tags = load_h5py_to_np('data/anime_face_one_int_label.h5')
    # generator = load_model('C:\Study\Python\LHYGANMLDS18\HW3_2\data\models\CGAN\g\\cgan1_relu32_g_9500.h5')
    # save_imgs(generator,tags)
